class FlashSprite extends GameSprite {
    #_PV = {};

    // ctor
    constructor(name, scene, frameData, atlasName, frameName, exhaustedCallback) {
    ...
    getSprite() {
        try {
            ...
            if (v.sprite == undefined) {
                v.sprite = v.scene.add.sprite(0, 0, v.atlasName, v.frameName);
            }
     ...
     flash(x, y) {
        try {
            ...
            var timer = new TimerOnPool('timeronpool_' + this.Name, v.scene.getTimerPool());
            timer.startTimeout(()=>{
                selfIt.alpha = 0;
                selfIt.visible = false;
                v.exhaustedCallback(selfIt);
            }, 200);

        } catch (e) {
            var errMsg = this.getExpMsg("flash", e);
            console.log(errMsg);
            alert(errMsg);
        }
     }
